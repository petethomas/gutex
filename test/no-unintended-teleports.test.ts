...
describe('3D Mode Teleport Boundary Logic', () => {
  ...
  describe('Forward boundary teleport conditions', () => {
    ...
    it('should require active forward scrolling to trigger teleport', () => {
      // Simulating the condition check from the animation loop
      const momentum = 0.0; // User is NOT scrolling
      ...
      const isScrollingForward = momentum > 0.05 || (autoReadActive && autoDirection === 'forward');
      const atForwardBoundary = wordOffset >= allWordsLength - 2 && 
        nextByteStart == null &&
        isScrollingForward;
      
      assert.strictEqual(isScrollingForward, false, 'Should not detect forward scroll with no momentum');
      assert.strictEqual(atForwardBoundary, false, 'Should NOT trigger teleport without active scrolling');
    });

    it('should trigger teleport when actively scrolling forward at end', () => {
      const momentum = 0.1; // User IS scrolling forward
      const autoReadActive = false;
      const autoDirection = 'forward';
      const wordOffset = 100;
      const allWordsLength = 100;
      ...
      assert.strictEqual(isScrollingForward, true, 'Should detect forward scroll with positive momentum');
      assert.strictEqual(atForwardBoundary, true, 'Should trigger teleport when actively scrolling at end');
    });

    it('should trigger teleport during auto-read forward at end', () => {
      const momentum = 0.0; // No manual momentum
      ...
      assert.strictEqual(isScrollingForward, true, 'Should detect auto-read forward');
      assert.strictEqual(atForwardBoundary, true, 'Should trigger teleport during auto-read at end');
    });

    it('should NOT trigger teleport when not at end even if scrolling', () => {
      const momentum = 0.2; // Scrolling forward
      const autoReadActive = false;
      const autoDirection = 'forward';
  ...
  describe('Backward boundary teleport conditions', () => {
    ...
    it('should require active backward scrolling to trigger teleport', () => {
      const momentum = 0.0; // User is NOT scrolling
      const autoReadActive = false;
      const autoDirection = 'backward';
      const wordOffset = 0;
      const firstByteStart = 1000;
      const docStart = 1000; // At document start
      
      const isScrollingBackward = momentum < -0.05 || (autoReadActive && autoDirection === 'backward');
      const atDocStart = firstByteStart <= docStart + 100;
      const atWordStart = wordOffset <= 1;
      const atBackwardBoundary = atWordStart && atDocStart && isScrollingBackward;
      
      assert.strictEqual(isScrollingBackward, false, 'Should not detect backward scroll with no momentum');
      assert.strictEqual(atBackwardBoundary, false, 'Should NOT trigger teleport without active scrolling');
    });

    it('should trigger teleport when actively scrolling backward at start', () => {
      const momentum = -0.1; // User IS scrolling backward
      const autoReadActive = false;
      const autoDirection = 'backward';
      const wordOffset = 0;
      const firstByteStart = 1000;
      ...
      assert.strictEqual(isScrollingBackward, true, 'Should detect backward scroll with negative momentum');
      assert.strictEqual(atBackwardBoundary, true, 'Should trigger teleport when actively scrolling at start');
    });
    ...
    it('should trigger teleport during auto-read backward at start', () => {
      ...
      assert.strictEqual(isScrollingBackward, true, 'Should detect auto-read backward');
      assert.strictEqual(atBackwardBoundary, true, 'Should trigger teleport during auto-read at start');
    });

    it('should NOT trigger when not at start even if scrolling backward', () => {
      const momentum = -0.2; // Scrolling backward
      const autoReadActive = false;
      const autoDirection = 'backward';
      const wordOffset = 50; // Middle of content
      const firstByteStart = 1000;
  ...
  describe('Mode toggle scenarios', () => {
    ...
    it('should not trigger teleport when toggling to 3D at end of book', () => {
      ...
      // momentum starts at 0, so should NOT teleport
      const momentum = 0; // Fresh toggle = no momentum
      const autoReadActive = false;
      const wordOffset = 100;
      const allWordsLength = 100;
      const nextByteStart: number | null = null;
      
      const isScrollingForward = momentum > 0.05 || (autoReadActive && false);
      const atForwardBoundary = wordOffset >= allWordsLength - 2 && 
        nextByteStart == null &&
        isScrollingForward;
    ...
    it('should not trigger teleport when toggling to 3D at start of book', () => {
      ...
      const isScrollingBackward = momentum < -0.05 || (autoReadActive && false);
      const atDocStart = firstByteStart <= docStart + 100;
      const atWordStart = wordOffset <= 1;
      const atBackwardBoundary = atWordStart && atDocStart && isScrollingBackward;
