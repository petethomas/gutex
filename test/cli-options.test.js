...
describe('CliOptions', () => {
  ...
  describe('validation errors', () => {
    it('should reject wrong number of arguments', () => {
      const options = new CliOptions(['996', '7']);
      
      assert.strictEqual(options.isValid(), false);
      assert.ok(options.errors.length > 0);
      assert.ok(options.getErrorMessage().includes('3 positional arguments'));
    });
    ...
    it('should reject non-numeric bookId', () => {
      ...
      assert.strictEqual(options.isValid(), false);
      assert.ok(options.errors.some(e => e.includes('must be numbers')));
    });
  ...
  describe('usage message', () => {
    it('should provide usage message', () => {
      ...
      assert.ok(usage.includes('--snapshot'));
      assert.ok(usage.includes('--raw'));
